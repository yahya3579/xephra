...
const { transporter, transactionalEmailApi } = require("../config/emailConfig");
...
// Send verification email function using Brevo
const sendVerificationEmail = async (user, verificationToken) => {
  const verificationUrl = `${process.env.FRONTEND_URL || 'http://localhost:3000'}/verify-email/${verificationToken}`;
  
  try {
    // Using Brevo API for sending verification email
    await transactionalEmailApi.sendTransacEmail({
      sender: {
        email: process.env.EMAIL_FROM_ADDRESS || 'noreply@xephra.net',
        name: 'Xephra'
      },
      to: [
        {
    ...
    console.log(`Verification email sent to ${user.email} using Brevo`);
  } catch (error) {
    ...
    try {
      ...
      console.log(`Verification email sent to ${user.email} using fallback method`);
    } catch (fallbackError) {
